...
class ChartViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        setBarChart(name: _arrThang, value: _arrSoLuong)
        // Do any additional setup after loading the view.
    }
    func setBarChart(name:Array<String>, value: Array<Double>)
    {
        var dataArray:[BarChartDataEntry]=[]
        for i in 0..<name.count{ let data: BarChartDataEntry = BarChartDataEntry(x:Double(i), yValues:[value[i]])
            dataArray.append(data)
        }
        let dataset: BarChartDataSet = BarChartDataSet(values:dataArray, label:"So Luong")
